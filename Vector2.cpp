...
double Vector2::getDistance(const Vector2& target) const
{
...
{
	return Vector2((float)(target.x - this->x > 0) - (target.x - this->x < 0), (float)(target.y - this->y > 0) - (target.y - this->y < 0));
}